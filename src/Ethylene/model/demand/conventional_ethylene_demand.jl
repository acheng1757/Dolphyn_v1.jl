...
```
"""
function conventional_ethylene_demand(EP::Model, inputs::Dict, setup::Dict)

    println(" -- Conventional Ethylene Module")
    ...
    @variable(EP, vConv_Ethylene_Demand[t = 1:T, z = 1:Z] >= 0 )

    #Ethylene Balance
    EP[:eEthyleneBalance] += vConv_Ethylene_Demand
    
    ### Expressions ###
    #Cost of Conventional Ethylene
   

    @expression(EP, eTotalConv_Ethylene_VarOut_Z[z = 1:Z], sum((inputs["omega"][t] * inputs["Ethylene_Prices"][t,z] * vConv_Ethylene_Demand[t,z]) for t in 1:T))

    @expression(EP, eTotalConv_Ethylene_VarOut, sum(EP[:eTotalConv_Ethylene_VarOut_Z][z] for z in 1:Z))

    #Add to objective function
    EP[:eObj] += eTotalConv_Ethylene_VarOut
