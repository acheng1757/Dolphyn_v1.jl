...
"""
function write_ethylene_outputs(EP::Model, genx_path::AbstractString, setup::Dict, inputs::Dict)
  ...
  if !haskey(setup, "OverwriteResults") || setup["OverwriteResults"] == 1
    ...
    path = joinpath(genx_path, "Results_Ethylene");
  if !(isdir(path))
          mkpath(path)
      end
  else
    # Find closest unused ouput directory name
    path = choose_ethylene_output_dir(genx_path)
    # Create directory if it does not exist
    if !(isdir(path))
        mkpath(path)
    end
  ...
  write_Ethylene_demand_balance(path, sep, inputs, setup, EP)
  write_ethylene_emissions(path,sep,inputs, setup, EP)
  write_ethylene_costs(path, sep, inputs, setup, EP)
  ...
  if setup["ModelEthyleneProduction"] == 1
    write_ethylene_production_balance(path, sep, inputs, setup, EP)
    write_ethylene_production_capacity(path, sep, inputs, setup, EP)
  end

  ## Print confirmation
  println("Wrote Ethylene outputs to $path$sep")
