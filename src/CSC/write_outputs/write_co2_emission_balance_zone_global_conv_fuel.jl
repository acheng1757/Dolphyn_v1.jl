	...
	for z in 1:Z
	   	...
	   	for t in 1:T
			...
			if setup["ModelEthyleneProduction"] == 1
				dfTemp1[t+rowoffset,21] = value(EP[:eEthylene_Production_CO2_Emissions_By_Zone][z,t])
			end
		...
		if setup["ModelEthyleneProduction"] == 1
			dfTemp1[t+rowoffset,21] = sum(inputs["omega"][t] * value.(EP[:eEthylene_Production_CO2_Emissions_By_Zone][z,t]) for t in 1:T)
		end
