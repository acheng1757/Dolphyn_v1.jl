...
function write_co2_emission_balance_system(path::AbstractString, inputs::Dict, setup::Dict, EP::Model)
    ...
    dfTemp1[1,1:size(dfTemp1,2)] = ["Power Emissions", "HSC Emissions", "CSC Emissions",  "Biorefinery Emissions", "Bioresource Emissions", "Biomass Capture", "Conventional NG", "Syn NG Plant Emissions", "Syn NG", "Bio NG", "Conventional Liquid Fuels", "Synfuel Plant Emissions" ,"Synfuels", "Biofuels", "Ethylene Emissions","Total"]
    for t in 1:T
        ...
        if setup["ModelEthyleneProduction"] == 1
            dfTemp1[t+rowoffset,15] = value(sum(EP[:eEthylene_Production_CO2_Emissions_By_Zone][z,t] for z in 1:Z))
        end
        ...
        if setup["ModelNGSC"] == 1
            ...
            if setup["ModelSyntheticNG"] == 1
                dfTemp1[t+rowoffset,8] = value(sum(EP[:eSyn_NG_Production_CO2_Emissions_By_Zone][z,t] for z in 1:Z))
                dfTemp1[t+rowoffset,9] = value(sum(EP[:eSyn_NG_CO2_Emissions_By_Zone][z,t] for z in 1:Z))
            end

            if setup["ModelBESC"] == 1 && setup["Bio_NG_On"] == 1
                dfTemp1[t+rowoffset,10] = value(sum(EP[:eBio_NG_CO2_Emissions_By_Zone][z,t] for z in 1:Z))
            end
        end
        ...
        if setup["ModelLFSC"] == 1
            if setup["Liquid_Fuels_Regional_Demand"] == 1 && setup["Liquid_Fuels_Hourly_Demand"] == 1
                dfTemp1[t+rowoffset,11] = value(sum(EP[:eConv_Diesel_CO2_Emissions][z,t] for z in 1:Z)) + value(sum(EP[:eConv_Jetfuel_CO2_Emissions][z,t] for z in 1:Z)) + value(sum(EP[:eConv_Gasoline_CO2_Emissions][z,t] for z in 1:Z))
            ...
            elseif setup["Liquid_Fuels_Regional_Demand"] == 0 && setup["Liquid_Fuels_Hourly_Demand"] == 1
                dfTemp1[t+rowoffset,11] = value(EP[:eConv_Diesel_CO2_Emissions][t]) + value(EP[:eConv_Jetfuel_CO2_Emissions][t]) + value(EP[:eConv_Gasoline_CO2_Emissions][t])
            ...
            if setup["ModelSyntheticFuels"] == 1
                dfTemp1[t+rowoffset,12] = value(sum(EP[:eSynfuels_Production_CO2_Emissions_By_Zone][z,t] for z in 1:Z)) + value(sum(EP[:eByProdConsCO2EmissionsByZone][z,t] for z in 1:Z))
                dfTemp1[t+rowoffset,13] = value(sum(EP[:eSyn_Diesel_CO2_Emissions_By_Zone][z,t] for z in 1:Z)) + value(sum(EP[:eSyn_Jetfuel_CO2_Emissions_By_Zone][z,t] for z in 1:Z)) + value(sum(EP[:eSyn_Gasoline_CO2_Emissions_By_Zone][z,t] for z in 1:Z))
            end

            if setup["ModelBESC"] == 1
                dfTemp1[t+rowoffset,14] = value(sum(EP[:eBio_Diesel_CO2_Emissions_By_Zone][z,t] for z in 1:Z)) + value(sum(EP[:eBio_Jetfuel_CO2_Emissions_By_Zone][z,t] for z in 1:Z)) + value(sum(EP[:eBio_Gasoline_CO2_Emissions_By_Zone][z,t] for z in 1:Z))
            end
        end
    ...
    if setup["ModelNGSC"] == 1
        ...
        if setup["ModelSyntheticNG"] == 1
            dfTemp1[rowoffset,8] = sum(sum(inputs["omega"].* value.(EP[:eSyn_NG_Production_CO2_Emissions_By_Zone])[z,:]) for z in 1:Z)
            dfTemp1[rowoffset,9] = sum(sum(inputs["omega"].* value.(EP[:eSyn_NG_CO2_Emissions_By_Zone])[z,:]) for z in 1:Z)
        end

        if setup["ModelBESC"] == 1 && setup["Bio_NG_On"] == 1
          dfTemp1[rowoffset,10] = sum(sum(inputs["omega"].* (value.(EP[:eBio_NG_CO2_Emissions_By_Zone])[z,:])) for z in 1:Z)
        end
    ...
    if setup["ModelEthyleneProduction"] == 1
        dfTemp1[rowoffset,15] = sum(sum(inputs["omega"].* value.(EP[:eEthylene_Production_CO2_Emissions_By_Zone])[z,:]) for z in 1:Z)
        # Consumption Emissions: dfTemp1[rowoffset,17] = sum(sum(inputs["omega"].* value.(EP[:eEthylene_CO2_Emissions_By_Zone])[z,:]) for z in 1:Z)
    end


    if setup["ModelLFSC"] == 1
        if setup["Liquid_Fuels_Regional_Demand"] == 1 && setup["Liquid_Fuels_Hourly_Demand"] == 1 
            dfTemp1[rowoffset,11] = sum(sum(inputs["omega"].* (value.(EP[:eConv_Diesel_CO2_Emissions][z,:]) + value.(EP[:eConv_Jetfuel_CO2_Emissions][z,:]) + value.(EP[:eConv_Gasoline_CO2_Emissions][z,:]))) for z in 1:Z)

        elseif setup["Liquid_Fuels_Regional_Demand"] == 1 && setup["Liquid_Fuels_Hourly_Demand"] == 0
            dfTemp1[rowoffset,11] = sum(value.(EP[:eConv_Diesel_CO2_Emissions][z]) for z in 1:Z) + sum(value.(EP[:eConv_Jetfuel_CO2_Emissions][z]) for z in 1:Z) + sum(value.(EP[:eConv_Gasoline_CO2_Emissions][z]) for z in 1:Z)

        elseif setup["Liquid_Fuels_Regional_Demand"] == 0 && setup["Liquid_Fuels_Hourly_Demand"] == 1
            dfTemp1[rowoffset,11] = sum(inputs["omega"].* value.(EP[:eConv_Diesel_CO2_Emissions][:])) + sum(inputs["omega"].* value.(EP[:eConv_Jetfuel_CO2_Emissions][:])) + sum(inputs["omega"].* value.(EP[:eConv_Gasoline_CO2_Emissions][:]))
       
        elseif setup["Liquid_Fuels_Regional_Demand"] == 0 && setup["Liquid_Fuels_Hourly_Demand"] == 0
            dfTemp1[rowoffset,11] = value.(EP[:eConv_Diesel_CO2_Emissions]) + value.(EP[:eConv_Jetfuel_CO2_Emissions]) + value.(EP[:eConv_Gasoline_CO2_Emissions])

        end
        
        if setup["ModelSyntheticFuels"] == 1
            dfTemp1[rowoffset,12] = sum(sum(inputs["omega"].* (value.(EP[:eSynfuels_Production_CO2_Emissions_By_Zone])[z,:] + value.(EP[:eByProdConsCO2EmissionsByZone])[z,:])) for z in 1:Z)
            dfTemp1[rowoffset,13] = sum(sum(inputs["omega"].* (value.(EP[:eSyn_Diesel_CO2_Emissions_By_Zone])[z,:] + value.(EP[:eSyn_Jetfuel_CO2_Emissions_By_Zone])[z,:] + value.(EP[:eSyn_Gasoline_CO2_Emissions_By_Zone])[z,:])) for z in 1:Z)
        end

        if setup["ModelBESC"] == 1
            dfTemp1[rowoffset,14] = sum(sum(inputs["omega"].* (value.(EP[:eBio_Diesel_CO2_Emissions_By_Zone])[z,:] + value.(EP[:eBio_Jetfuel_CO2_Emissions_By_Zone])[z,:] + value.(EP[:eBio_Gasoline_CO2_Emissions_By_Zone])[z,:])) for z in 1:Z)
        end
    end

    
    dfTemp1[rowoffset,16] =  dfTemp1[rowoffset,1] + dfTemp1[rowoffset,2] + dfTemp1[rowoffset,3] + dfTemp1[rowoffset,4] + dfTemp1[rowoffset,5] + dfTemp1[rowoffset,6] + dfTemp1[rowoffset,7] + dfTemp1[rowoffset,8] + dfTemp1[rowoffset,9] + dfTemp1[rowoffset,10] + dfTemp1[rowoffset,11] + dfTemp1[rowoffset,12] + dfTemp1[rowoffset,13] + dfTemp1[rowoffset,14] + dfTemp1[rowoffset,15]
